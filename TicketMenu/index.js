...
client.on("guildMemberAdd", (member) => {
    let canal_logs = ""; //id do canal de logs
    if (!canal_logs) return;

    let setado = member.guild.roles.cache.get("") //id do canal de logs
...
client.on("interactionCreate", (interaction, member) => {
    if (interaction.isSelectMenu()) {
      if (interaction.customId === "painel_ticket") {
        let member = "||<@>||" //ID DO DONO EX <@131231>
        let opc = interaction.values[0]
        if (opc === "t1") {
  
          let nome = `📨-${interaction.user.id}`;
          let categoria = "" // Coloque o ID da categoria dos tickets
          ...
          if (interaction.guild.channels.cache.find(c => c.name === nome)) {
            ...
            let embed = new Discord.EmbedBuilder()
            .setColor("#a40000")
            .setDescription(`Olá ${interaction.user}, Você solicitou um ticket em **PT-BR**, Entraremos em contato o mais rápido possível, aguarde. Clique no botão vermelho para encerrar o ticket.`);
          ...
          if (interaction.guild.channels.cache.find(c => c.name === nome)) {
            ...
            interaction.reply({ content: `✅ Olá ${interaction.user}, your ticket was opened on ${ch}!`, ephemeral: true })
            let embed = new Discord.EmbedBuilder()
            .setColor("#a40000")
            .setDescription(`Hello ${interaction.user}, You requested a ticket in **EN-US**, We will contact you as soon as possible, please wait. Click the red button to close the ticket.`);
            let botao = new Discord.ActionRowBuilder().addComponents(
              new Discord.ButtonBuilder()
            .setCustomId("fechar_ticket")
            .setEmoji("🔒")
